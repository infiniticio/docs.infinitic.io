...
- When this method is dispatched (from a [client](/docs/introduction/terminology#client) or in a [Workflow](/docs/introduction/terminology#worker) worker), the argument will be serializable as a `Foo`, and the output will be deserialized as a `Bar`.
- When this method is processed (in a [Service](/docs/introduction/terminology#worker) or [Workflow](/docs/introduction/terminology#worker) worker), the received data will be deserialized as a `Foo`, and the output will be serialized as a `Bar`.
...
To verify if `Foo.class` is properly serializable, use the following test pattern with `foo` being an instance of `Foo` (the same applies for `Bar.class`) :
...
1. Concrete Class: The above pattern works directly for concrete classes.

2. Polymorphic Class: If `Foo` is an abstract class, that have 2 derivated class `FooA` and `FooB`, add Jackson annotations to provide type information:
...
3. Interfaces: If `Foo` is an interface, implemented by classes `FooA` and `FooB`, add Jackson annotations to provide type information:
...
- This customization affects all subsequent serialization/deserialization operations in the current JVM.
...
If `kotlinx-serialization-json` is not used (i.e. your class does not have a serializer), the fallback serialization/deserialization method will be [FasterXML/jackson](https://github.com/FasterXML/jackson-docs) as for Java.
...
1. Concrete Class: The easiest way is to use [data classes](https://kotlinlang.org/docs/reference/data-classes.html) with a `kotlinx.serialization.Serializable` annotation. For example :
...
2. Polymorphic Class: If `Foo` is an abstract class, the easiest way to use a `sealed class`:
    ```kotlin
    import kotlinx.serialization.Serializable
    ...
    If `Foo` is not sealed, you need to provide the polymorphic info to the serializer. For example if `Foo` has 2 derivated class `FooA` and `FooB`:
...
3. Interfaces: If `Foo` is an interface, the easiest way to use a `sealed interface`:
    ...
    If `Foo` is not sealed, you need to provide the polymorphic info to the serializer. For example, if `Foo` is implemented by classes `FooA` and `FooB`:
...
Key points:
- Update mutable property `io.infinitic.serDe.kotlin.json`.
- This customization affects all subsequent serialization/deserialization operations in the current JVM.
- Ensure it is used consistently in both clients and workers
- Ensure it is used consistently with existing messages
